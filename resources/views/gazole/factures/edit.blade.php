    ...
    <form action="{{ route('factures.update', $facture->id) }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <input type="date" class="form-control" value="<?php echo date('Y-m-d'); ?>" name="date" value="{{ $facture->date }}"
                    aria-describedby="emailHelp">
            </div>
            <div class="col-6">
                ...
                <select name="station_id" id="citySelect"class="form-select">
                    @foreach (\App\Models\Station::all() as $item)
                        <option value="{{ $item->id }}" @if ($item->id = $facture->station_id) selected @endif>{{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-6">
                <label for="exampleInputEmail1" class="form-label">total</label>
                <input type="text" class="form-control" id="exampleInputEmail1" name="prix" value="{{ $facture->prix }}"
                    aria-describedby="emailHelp">
            </div><br><br><br><br>
        </div>
